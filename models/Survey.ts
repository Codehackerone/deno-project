...
export default class Survey extends BaseModel {
  ...
  static async findAll(): Promise<Survey[]> {
    const surveys = surveysCollection.find({}, {
  ...
  static async findByUser(userId: string): Promise<Survey[]> {
    let surveys = surveysCollection.find({ userId }, {
      noCursorTimeout: false,
    } as any);
    return surveys.map((survey: any) => Survey.prepare(survey));
  }
  ...
  static async findById(id: string): Promise<Survey | null> {
    try {
      let survey = await surveysCollection.findOne(
  ...
  async update({
    ...
    const { modifiedCount } = await surveysCollection.updateOne(
      { _id: this.id },
      {
